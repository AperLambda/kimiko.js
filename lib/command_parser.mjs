 ...
 * 
 * @param {CommandArgument[]} cmd_args The command arguments.
 * @param {String} args The arguments as a string.
 * @return An array of the parsed arguments with as key the name of the argument.
 */
export function parse_arguments(cmd_args, args) {
    let required_args = cmd_args.filter(arg => arg.required).length;
    ...
    let result = [];
    args.forEach((arg, i) => {
        let cmd_arg = cmd_args[i];
        let list = false;
        if (i >= cmd_args.length) {
            cmd_arg = cmd_args[cmd_args.length - 1];
            if (!cmd_arg.list)
                throw new SyntaxError(cmd, `Too many arguments provided (${args.length}), max: (${cmd_args.length}).`, "kimiko.js ; parse_arguments(cmd,args): ");
            list = true;
