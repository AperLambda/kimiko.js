...
const Command = module.exports = class Command
{
  ...
  execute(context, label, args = [])
  {
    ...
    return this.executor.execute(context, this, label, args);
  }
  ...
  onTabComplete(context, label, args = [])
  {
    if (args.length == 1) {
      ...
      if (!subCommands.length) {
        return this.tabCompleter.onTabComplete(context, this, label, args);
      }
      ...
      let additionalCompletion = this.tabCompleter.onTabComplete(context, this, label, args);
  ...
  hasSubCommand(subCommand = "")
  {
    if (typeof subCommand == "object") {
      return !!(this.subCommands.includes(subCommand) || this.subCommands.find((sc) => sc.getName() === subCommand.getName()));
    }
    else if (typeof subCommand == "string") {
      return this.subCommands.find((sc) => sc.getName() === subCommand);
    }
    return false;
  }
  ...
  removeSubCommand(subCommand)
  {
    ...
    if (typeof subCommand == "string") {
      index = this.subCommands.findIndex((sc) => sc.getName() === name);
    }
    else if (typeof subCommand == "object") {
      index = this.subCommands.indexOf(subCommand);
    }
  ...
  getSubCommand(label)
  {
    ...
    return this.subCommands.find((sc) => sc.getName().toLowerCase() === label);
  }
