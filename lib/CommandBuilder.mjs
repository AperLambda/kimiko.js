...
export default class CommandBuilder 
{
    ...
    build() {
        ...
        if (!this._handler) {
            throw new Error(`kimiko.js ; CommandBuilder#build(): CommandBuilder::_handler hasn't been defined yet.`);
        }
        cmd.handler = this._handler;
        if (this._description) cmd.description = this._description;
        if (this._permission) cmd.permission = this._permission;
        if (this._aliases) cmd.aliases = this._aliases;
        this.subcommands.forEach(sub => cmd.add_sub_command(sub));
        return cmd;
    }
